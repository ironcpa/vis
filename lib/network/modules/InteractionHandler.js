...
class InteractionHandler {
  ...
  onDragStart(event) {
    ...
    let node = this.selectionHandler.getNodeAt(this.drag.pointer)
      || this.selectionHandler.getGroupAt(this.drag.pointer);

    // hjchoi.resize
    // console.log('onDragStart', this.drag.pointer, node.x, node.y, node)
    let canvasPos = this.canvas.DOMtoCanvas(this.drag.pointer);
    if (node !== undefined && node.options.shape == 'groupBox') {
      let handlePos = node.shape.isOnResizeHandle(canvasPos)
      if (handlePos) {
        this.resizeTarget = {
          id: node.id,
          x: node.x,
        ...
        this.resizeTarget.node.resizing = true;
        return;
      }
    }
    ...
    if (node !== undefined && this.options.dragNodes === true) {
      ...
      // hjchoi:move-group
      if (node.options.shape == 'groupBox') {
        let children = this.selectionHandler.getAllNodesIn(node.shape);
        this.groupChildren = [];
   ...
   */
  onDrag(event) {
    // console.log('onDrag', this.drag.pointer)
    ...
    if (this.resizeTarget) {
      let node = this.resizeTarget.node;
      let handlePos = this.resizeTarget.handlePos;
      if (handlePos) {
        ...
        let w = 0, h = 0;
        if (handlePos.x == 'left') {
          w = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(this.resizeTarget.width) - deltaX);
        } else if (handlePos.x == 'right') {
          w = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(this.resizeTarget.width) + deltaX);
        }
        if (handlePos.y == 'top') {
          h = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(this.resizeTarget.height) - deltaY);
        } else if (handlePos.y == 'bottom') {
          h = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(this.resizeTarget.height) + deltaY);
        }

        let min_w = 20, min_h = 20; 
        if (w < min_w || h < min_h) {
          return;
        }
        ...
        node.shape.width = w;
        node.shape.height = h;
        node.options.width = w;
        node.options.height = h;
        node.x = this.resizeTarget.x + deltaX / scale / 2;  // apply scale factor
        node.y = this.resizeTarget.y + deltaY / scale / 2;  // apply scale factor
        // console.log('node(x,y)', node.x, node.y, scale)
      }
    ...
    if (selection && selection.length && this.options.dragNodes === true) {
      ...
      let shiftKey = event.srcEvent.shiftKey;
      selection.forEach((selection) => {
        let node = selection.node;
        // only move the node if it was not fixed initially
        if (selection.xFixed === false) {
          let newX = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(selection.x) + deltaX);
          if (shiftKey)
            node.x = Math.floor(newX / 10) * 10
          else
            node.x = newX
        }
        // only move the node if it was not fixed initially
        if (selection.yFixed === false) {
          let newY = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(selection.y) + deltaY);
          if (shiftKey)
            node.y = Math.floor(newY / 10) * 10
          else
            node.y = newY
        }
        // console.log('onDrag: new node.xy', selection.xFixed, node.x, node.y)

        // hjchoi.group_move
        if (node.options.shape == 'groupBox') {
          for (let nodeId in this.groupChildren) {
            if ({}.hasOwnProperty.call(this.groupChildren, nodeId)) {
              let child = this.groupChildren[nodeId]
              child.node.x = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(child.x) + deltaX);
              child.node.y = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(child.y) + deltaY);
            }
          }
        }
      });
   ...
   */
  onDragEnd(event) {
    // console.log('onDragEnd', this.drag.pointer)
    if (this.resizeTarget) {
      this.resizeTarget.node.resizing = false;
