...
class InteractionHandler {
  ...
  onDragStart(event) {
    ...
    if (node !== undefined && node.options.shape == 'groupBox' && node.shape.isOnResizeHandle(canvasPos)) {
      this.resizeTarget = {
        id: node.id,
        x: node.x,
        y: node.y,
  ...
  onDrag(event) {
    ...
    if (this.resizeTarget) {
      ...
      let w = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(this.resizeTarget.width) + deltaX);
      let h = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(this.resizeTarget.height) + deltaY);
      node.shape.width = w;
      node.shape.height = h;
