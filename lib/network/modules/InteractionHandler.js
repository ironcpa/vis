...
class InteractionHandler {
  ...
  onDragStart(event) {
    ...
    if (node.options.shape == 'groupBox' && node.shape.isOnResizeHandle(canvasPos)) {
      this.resizeTarget = node;
      this.resizeTarget.resizing = true;
      return;
    }
  ...
  onDrag(event) {
    ...
    if (this.resizeTarget) {
      // draw resizing box on rendering
      return;
    }
  ...
  onDragEnd(event) {
    ...
    if (this.resizeTarget) {
      ...
      // hjchoi.resize: todo: fix to fit w/ mouse pos when zoom in/out, below is not working
      // let w = this.canvas._XconvertDOMtoCanvas(this.canvas._XconvertCanvasToDOM(this.resizeTarget.shape.width) + deltaX);
      // let h = this.canvas._YconvertDOMtoCanvas(this.canvas._YconvertCanvasToDOM(this.resizeTarget.shape.height) + deltaY);
      let w = this.resizeTarget.shape.width + deltaX;
      let h = this.resizeTarget.shape.height + deltaY;

      // hjchoi.resize: todo: change to modify single field liek 'wiidth/height'
      this.resizeTarget.shape.width = w;      // for resize during dragging
      this.resizeTarget.shape.height = h;
      this.resizeTarget.options.width = w;    // for resize after drag
      this.resizeTarget.options.height = h;
      this.resizeTarget.x = this.resizeTarget.x + deltaX / 2;   // for adjust position, cuz canvas (x,y) is center based
      this.resizeTarget.y = this.resizeTarget.y + deltaY / 2;

      this.resizeTarget.resizing = false;
