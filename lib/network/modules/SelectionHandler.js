...
class SelectionHandler {
  ...
  selectOnPoint(pointer) {
    ...
    if (this.options.selectable === true) {
      let obj = this.getNodeAt(pointer) || this.getEdgeAt(pointer) || this.getGroupAt(pointer);
   ...
   * proiority when overlapping
   *   - last one except groupbox
   *   - if there's only 1 node on pointer that it
   *
   * @param {{x: number, y: number}} pointer
  ...
  getNodeAt(pointer, returnNode = true) {
    ...
    // except 'groupBox' node : hjchoi 20191007
    if (overlappingNodes.length > 0) {
      ...
      for (let i = overlappingNodes.length - 1; i >= 0; i--) {
        // return first non groupBox node
        if (this.body.nodes[overlappingNodes[i]].options.shape == 'groupBox') {
          continue;
        } else {
          if (returnNode === true) {
            return this.body.nodes[overlappingNodes[i]];
          }
          else {
      ...
      // let lastNodeExceptGroupBox = 0;
      // if (overlappingNodes.length > 1) {
      //   for (let i = overlappingNodes.length - 1; i > 0; i--) {
      //     if (this.body.nodes[overlappingNodes[i]].options.shape == 'groupBox')
      //       continue;
      //     lastNodeExceptGroupBox = i;
      //     break;
      //   }
      // }
      // if (returnNode === true) {
      //   return this.body.nodes[overlappingNodes[lastNodeExceptGroupBox]];
      // }
      // else {
      //   return overlappingNodes[lastNodeExceptGroupBox];
      // }
  ...
  /**
   * @param {Object} object : comparing node
   * @return {Object[]} found nodes
   */
  getAllNodesIn(object) {
    let founds = []; let nodes = this.body.nodes;
    for (let i = 0; i < this.body.nodeIndices.length; i++) {
      let nodeId = this.body.nodeIndices[i];
      if (nodes[nodeId].isIn(object)) {
        founds.push(nodes[nodeId]);
      }
    }
    return founds;
  }
   ...
   * @return {Node | undefined} node
   */
  getGroupAt(pointer, returnNode = true) {
    // we first check if this is an navigation controls element
    let positionObject = this._pointerToPositionObject(pointer);
    let overlappingNodes = this._getAllNodesOverlappingWith(positionObject);
