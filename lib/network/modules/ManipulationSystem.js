...
class ManipulationSystem {
  ...
  _finishConnect(event) {
    ...
    let node = undefined;
    for (let i = overlappingNodeIds.length-1; i >= 0; i--) {
      // if the node id is NOT a temporary node and whyit groupbox node, accept the node.
      if (this.temporaryIds.nodes.indexOf(overlappingNodeIds[i]) === -1
        && this.body.nodes[overlappingNodeIds[i]].options.shape != 'groupBox') {
        node = this.body.nodes[overlappingNodeIds[i]];
        break;
      }
    }
